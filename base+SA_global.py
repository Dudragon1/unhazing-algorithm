...
class GLU_layer(nn.Module):
	def __init__(self, dim, kernel_size=5, gate_act=nn.Sigmoid):
		super().__init__()
		self.dim = dim
...
class TransformerBlock(nn.Module):
    def __init__(self, network_depth, dim, num_heads, mlp_ratio=4.,
        ...
        self.GSA = GSA_layer(dim, dim) if self.last else nn.Identity()
        #################
...
class SFconv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.stage == 3:
            # x = [E1, E2, E3, D3] = [24, 24, 48, 48]
            x_E1 = self.Down_DWC(x[0])
            x_E2 = self.Down_DWC(x[1])
            x_E3 = self.con1(x[2])
            x_D3 = x[3]
            emerge = x_E1 + x_E2 + x_E3 + x_D3
            x_value = torch.cat([x_E1, x_E2, x_E3, x_D3], dim=1)
        if self.stage == 2:
            # x = [E1, E2, D2] = [24, 24, 24]
            x_E1 = self.con1(x[0])
            x_E2 = self.con1(x[1])
            x_D2 = x[2]
            emerge = x_E1 + x_E2 + x_D2
            x_value = torch.cat([x_E1, x_E2, x_D2], dim=1)
