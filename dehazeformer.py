...
class DehazeFormer(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, window_size=8,
        ...
        self.FAM = FAM(embed_dims[1])
        self.SCM = SCM(embed_dims[1])


        self.ConvsOut = BasicConv(embed_dims[3], 3, kernel_size=3, relu=False, stride=1)
    ...
    def forward_features(self, x):
        x_2 = F.interpolate(x, scale_factor=0.5)    # 缩小一半的x
        x2 = self.SCM(x_2)                         # 将x_2通过卷积

        outputs = list()
        x = self.patch_embed(x)     # 映射 24
        x = self.layer1(x)
        skip1 = x

        x = self.patch_merge1(x)    # 下采样 48
        x = self.FAM(x, x2)
        x = self.layer2(x)
        skip2 = x

        x = self.patch_merge2(x)    # 下采样 96
        x = self.layer3(x)
        x = self.patch_split1(x)    # 上采样 48

        x = self.fusion1([x, self.skip2(skip2)]) + x
        x = self.layer4(x)
        x_ = self.ConvsOut(x)
        outputs.append(x_ + x_2)
        x = self.patch_split2(x)    # 上采样 24
    ...
    def forward(self, x):
        H, W = x.shape[2:]
        x = self.check_image_size(x)    # x = [inp, a]

        feat = self.forward_features(x)
        K, B = torch.split(feat[1], (1, 3), dim=1)
