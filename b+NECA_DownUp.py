...
class DAU(nn.Module):
    def __init__(
        ...
        self.PA = PALayer(channel=n_feat)

        self.down = nn.Conv2d(n_feat, n_feat, kernel_size=2, stride=2, groups=n_feat)
        # self.down = PatchEmbed(patch_size=2, in_chans=n_feat, embed_dim=n_feat*2)
    ...
    def forward(self, x):
        ...
        pa_branch_down = self.down(res)
        pa_branch_down = self.PA(pa_branch_down)
        pa_branch_up = F.interpolate(pa_branch_down, size=ca_branch.shape[2:], mode='bilinear')

        res = torch.cat([sa_branch, ca_branch, pa_branch_up], dim=1)
...
if __name__ == '__main__':
    ...
    # net = DehazeFormer(depths=[2, 2, 2, 2, 2]).to(device)
