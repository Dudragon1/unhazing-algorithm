...
class SFconv(nn.Module):
    def __init__(self, features, stage, M=4, r=8, L=4) -> None:
        ...
        # 深度可分离卷积
        self.con1 = nn.Conv2d(features, features, 3, padding=1, groups=features, padding_mode='reflect')
        # self.con1 = nn.Conv2d(features, features, 1)
        # self.con1 = nn.Sequential(
        #     nn.Conv2d(features, features, 3, padding=1, groups=features, padding_mode='reflect'),
    ...
    def forward(self, x):
        ...
        if self.stage == 3:
            ...
            # emerge = self.mix2(x_E1) + self.mix2(x_E2) + self.mix2(x_E3) + self.mix2(x_D3)
        if self.stage == 2:
            # x = [E1, E2, D2] = [24, 24, 24]
            x_E1 = x[0]
            x_E2 = x[1]
