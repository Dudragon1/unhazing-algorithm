...
class Fcm_layer(nn.Module):
    ...
    def forward(self, x):
        # 将x划分成四个不同频率分量
        lowlow = self.conv_HF(x)
        low = self.conv_HF(lowlow)
        middle = self.conv_HF(low)
        high = self.conv_HF(middle)
...
class DAU(nn.Module):
    def __init__(
            ...
            bias=False, bn=False, act=nn.PReLU(), res_scale=1, last=False, stage3=False):
...
class TransformerBlock(nn.Module):
    def __init__(self, network_depth, dim, num_heads, mlp_ratio=4.,
        ...
        #################
        self.DAU = DAU(n_feat=dim, act=nn.ReLU(), last=self.last, stage3=stage3)
...
class BasicLayer(nn.Module):
    def __init__(self, network_depth, dim, depth, num_heads, mlp_ratio=4.,
                 ...
                 attn_ratio=0., attn_loc='last', conv_type=None, last=False, stage3=False):
